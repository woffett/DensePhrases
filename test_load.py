...
def get_phrase_vecs(mips, questions, answers, outs, regex=False, max_answer_length=10, top_k=100):
    assert mips is not None
    ...
    start_vecs = np.reshape(start_vecs, (batch_size, top_k*2, -1))
    end_vecs = np.reshape(end_vecs, (batch_size, top_k*2, -1))
    phrase_vecs = np.reshape(phrase_vecs, (batch_size, top_k*2, max_answer_length, -1))

    return start_vecs, end_vecs, phrase_vecs, targets

def get_top_phrases(mips, questions, answers, query_encoder, tokenizer, batch_size, path, max_answer_length=10, top_k=100):
    # Search
    step = batch_size
    phrase_idxs = []
